...
self.addEventListener('install', (event) => {
    console.log('[SW] Installing v2 and caching static assets...');
    event.waitUntil(
...
self.addEventListener('fetch', (event) => {
    ...
    // 1. DATA STRATEGY: Supabase API (Network Only)
    // Avoid caching POST/PUT/DELETE or even GET for API to ensure fresh data
    if (url.hostname.includes('supabase.co')) {
        // We don't use respondWith here to let the browser handle it directly, 
        // OR we use it but ensure we don't return null
        return; 
    }

    // 2. ASSETS STRATEGY: External Libs (React, Lucide, Recharts, Fonts) - Cache First
        ...
        event.respondWith(
            caches.match(event.request).then((response) => {
                ...
                return fetch(event.request).then((networkResponse) => {
                    ...
                    // Return a generic failed response if network is down
    ...
    // 3. UI STRATEGY: Internal Assets & Index - Stale-While-Revalidate
    event.respondWith(
        caches.match(event.request).then((cachedResponse) => {
            const fetchPromise = fetch(event.request).then((networkResponse) => {
                ...
                // If fetch fails and no cache, don't return null
                return null;
            });

            // IMPORTANT: respondWith must always receive a Response object or a promise that resolves to one.
            // If we have no cache and fetch failed, we must at least return a basic error response.
